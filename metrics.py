...
def turbine_scores(pred, gt, raw_data, examine_len, stride=1):
    ...
    for i in range(0, cnt_sample, stride):
        ...
        maes.append(_mae)
        rmses.append(_rmse)
    avg_mae = np.array(maes)
    avg_mae = np.nanmean(avg_mae)

    avg_rmse = np.array(rmses)
    avg_rmse = np.nanmean(avg_rmse)
...
def regressor_detailed_scores(predictions, gts, raw_df_lst, settings):
    ...
    total_mae = np.array(all_mae)
    total_mae = np.nansum(total_mae)

    total_rmse = np.array(all_rmse)
    total_rmse = np.nansum(total_rmse)
